...
import 'package:task_manager_new/data/models/task_model.dart';
...
import 'package:task_manager_new/ui/widgets/centered_progress-indicator.dart';
...
class _TaskItemState extends State<TaskItem> {
      ...
      child: ListTile(
        ...
        subtitle: Column(
          ...
          children: [
            ...
            Text('Date: ${widget.taskModel.createdDate}', style: const TextStyle(
              color: Colors.black,
            ...
            Row(
              ...
              children: [
                Chip(label:  Text(widget.taskModel.status ??'New'), shape: RoundedRectangleBorder(
                        ...
                        icon: const Icon(Icons.edit),
                          ...
                          return statusList.map((String value){
                            return PopupMenuItem<String>(
                              value: value,
                              child: ListTile(title: Text(value), trailing: popupMenuValue == value ? const Icon(Icons.done) : null,),
  ...
  Future<void> _editTask() async{
    ...
    NetworkResponse response = await NetworkCaller.getRequest(Urls.updateTaskStatus(widget.taskModel.sId!, widget.taskModel.status!));
    if(response.isSuccess){
      widget.onUpdateTask;
      ...
      if(mounted){
        showSnackBarMessage(context, response.errorMessage ?? 'Edit task failed! Try again.');
      }
    }
    _deleteInProgress = false;
    if(mounted){
